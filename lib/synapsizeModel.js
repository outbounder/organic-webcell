...
module.exports = function(name, Model, Synapse, config) {
  ...
  if(!Model.synaptic) {
    ...
    Model.prototype.initialize = function(){
      ...
      if(config.debug) console.log("creating instance".green, config.debugData?this:"");
    }
    ...
    Model.prototype.free = function(){
      for(var i = 0; i<Model.$instances.length; i++) {
        if(Model.$instances[i] === this) {
          ...
          i -= 1;
          if(config.debug) console.log("freeing instance".green, config.debugData?this:"");
        }
      }
    ...
    Model.prototype.sync = function(method, model, options){
      if(config.debug) console.log("sync".red, method, config.debugData?model:"");
      Model.broadcast({method: method, model: model, options: options}, this, function(err, response){
        if(config.debug) console.log("sync:end".red, err, config.debugData?response:"");
    ...
    Model.broadcast = function(chemical, sender, callback) {
      ...
      if(config.debug) console.log(("broadcast").red, !config.debugData?chemical:"");
